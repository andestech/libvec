...
#define LEFT_SHIFT(a, b) ((q31_t)((unsigned long)(a) << (b)))

#define DESCALE(x, n) RIGHT_SHIFT((x) + ((int32_t)1 << ((n)-1)), n)

#ifdef DCT_FORMULA_SOL
u8_t test_cast(q31_t input)
{
    input += CENTERJSAMPLE;
    if (input < 0)
    {
...
riscv_vec_dct2d_8x8_cfg_q15_t riscv_vec_dct2d_8x8_fast_init_q15()
{
        u32_t memneeded = sizeof(riscv_vec_dct2d_8x8_state_q15_t)
                    + sizeof(u8_t) * 1024  /* idct_limit */
                    + sizeof(q31_t) * 64   /* buffer */
                    + NDSV_ALIGNMENT;      /* 64-bit alignment */

    riscv_vec_dct2d_8x8_cfg_q15_t ret = NDSV_MALLOC(memneeded);
    if (ret == NULL)
    {
        return NULL;
    }

    uintptr_t addr = (uintptr_t)ret + sizeof(riscv_vec_dct2d_8x8_state_q15_t);
...
riscv_vec_dct2d_8x8_cfg_q15_t riscv_vec_dct2d_8x8_init_q15()
{
        u32_t memneeded = sizeof(riscv_vec_dct2d_8x8_state_q15_t)
                    + sizeof(u8_t) * 1024  /* idct_limit */
                    + sizeof(q31_t) * 64   /* buffer */
...
static inline void vec_dct2d_8x8_q15_c(q15_t *dst, q15_t *src)
{
...
static inline void vec_idct2d_8x8_q15_c(u8_t *dst, q15_t *src, q15_t *dct_table, riscv_vec_dct2d_8x8_cfg_q15_t cfg)
{
...
void riscv_vec_dct2d_8x8_fast_q15(q15_t *dst, q15_t *src, riscv_vec_dct2d_8x8_cfg_q15_t cfg)
{
    if (cfg == NULL)
    {
        return;
    }    
    vec_dct2d_8x8_q15_c(dst, src);
}

void riscv_vec_idct2d_8x8_fast_q15(u8_t *dst, q15_t *src, q15_t *dct_table, riscv_vec_dct2d_8x8_cfg_q15_t cfg)
{
    if (cfg == NULL)
    {
        return;
    }  
    vec_idct2d_8x8_q15_c(dst, src, dct_table, cfg);
}

void riscv_vec_dct2d_8x8_q15(q15_t *dst, q15_t *src, riscv_vec_dct2d_8x8_cfg_q15_t cfg)
{
    if (cfg == NULL)
    {
        return;
...
#elif defined(DCT_FORMULA_SOL)
    q15_t *c = cfg->cos_table;
    ...
    for (int i = 0; i < 8; i++)
    {
        for (int j = 0; j < 8; j++)
        {
            ...
            tmp[8 * i + j] = (s) >> (FDCT_COS_BITS - FDCT_PASS1_BITS);
        }
    }
    ...
    for (int j = 0; j < 8; j++)
    {
        for (int i = 0; i < 8; i++)
        {
            ...
            dst[8 * i + j] = (s) >> (FDCT_COS_BITS + FDCT_PASS1_BITS);
        }
    }
#else // jpeg-turbo jpeg_fdct_islow
    vec_dct2d_8x8_q15_c(dst, src);
#endif
}

void riscv_vec_idct2d_8x8_q15(u8_t *dst, q15_t *src, q15_t *dct_table, riscv_vec_dct2d_8x8_cfg_q15_t cfg)
{
    if (cfg == NULL)
    {
        return;
    }    
#if defined(DCT_FORMULA_SOL)
    q15_t *c = cfg->idct_cos_table;
    q15_t tmp[64];
    ...
    for (int j = 0; j < 8; j++)
    {
        for (int i = 0; i < 8; i++)
        {
            ...
            dst[8 * i + j] = test_cast(((s) >> (FDCT_COS_BITS + FDCT_PASS1_BITS + 3)));
        }
    }
#else // jpeg-turbo jpeg_fdct_islow
    vec_idct2d_8x8_q15_c(dst, src, dct_table, cfg);
...
void riscv_vec_dct2d_8x8_free_q15(riscv_vec_dct2d_8x8_cfg_q15_t cfg)
{
    NDSV_FREE(cfg);
}
