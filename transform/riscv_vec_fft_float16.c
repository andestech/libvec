...
void riscv_vec_butterfly_f16 (riscv_vec_cpx_f16_t * FUNC_RESTRICT out,
...
void riscv_vec_cfft2d_f16 (riscv_vec_cpx_f16_t * FUNC_RESTRICT out,
                           riscv_vec_cpx_f16_t * FUNC_RESTRICT in,
                           ...
                           q31_t inverse_fft)
{
    ...
    if(cfg != NULL)
    {
        if(inverse_fft)
        {
            ...
            {
                tmp_out = (riscv_vec_cpx_f16_t*) &tmp_buff[tmp_idx * n];
                data_in = (riscv_vec_cpx_f16_t*) &in[tmp_idx * n];
                riscv_vec_inverse_butterfly_f16 (tmp_out, data_in, cfg->factors_n, cfg->twiddles_n, tmp_1d);
            }
            trans_in = (float16_t*) &tmp_buff[0].r;
            trans_out = (float16_t*) &out[0].r;
            riscv_vec_rmcmat_trans_f16(trans_in, trans_out, m, n);
            // 2nd: process m-dim 1d-fft for n times
            for(tmp_idx = 0; tmp_idx < n ; tmp_idx++)
            {
                tmp_out = (riscv_vec_cpx_f16_t*) &tmp_buff[tmp_idx * m];
                data_in = (riscv_vec_cpx_f16_t*) &out[tmp_idx * m];
                riscv_vec_inverse_butterfly_f16(tmp_out, data_in, cfg->factors_m, cfg->twiddles_m, tmp_1d);
            }
            trans_in = (float16_t*) &tmp_buff[0].r;
            trans_out = (float16_t *) &out[0].r;
            riscv_vec_rmcmat_trans_f16(trans_in, trans_out, n, m);
        }
        else
        {
            ...
            for(tmp_idx = 0 ; tmp_idx < m ; tmp_idx++)
            {
                tmp_out = (riscv_vec_cpx_f16_t*) &tmp_buff[tmp_idx * n];
                data_in = (riscv_vec_cpx_f16_t*) &in[tmp_idx * n];
                riscv_vec_butterfly_f16 (tmp_out, data_in, cfg->factors_n, cfg->twiddles_n, tmp_1d);
            }
            trans_in = (float16_t*) &tmp_buff[0].r;
            trans_out = (float16_t*) &out[0].r;
            riscv_vec_rmcmat_trans_f16(trans_in, trans_out, m, n);
            ...
            for(tmp_idx = 0; tmp_idx < n ; tmp_idx++)
            {
                tmp_out = (riscv_vec_cpx_f16_t*) &tmp_buff[tmp_idx * m];
                data_in = (riscv_vec_cpx_f16_t*) &out[tmp_idx * m];
                riscv_vec_butterfly_f16 (tmp_out, data_in, cfg->factors_m, cfg->twiddles_m, tmp_1d);
            }
