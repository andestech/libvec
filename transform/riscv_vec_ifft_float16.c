...
//CIFFT
void riscv_vec_inverse_butterfly_f16 (riscv_vec_cpx_f16_t * FUNC_RESTRICT out,
                                      ...
                                      riscv_vec_cpx_f16_t * FUNC_RESTRICT buffer)
{
    ...
    float16_t inv_factor = 1.0f / ((float16_t)(2 * input_m)); // 2*input_m = fft_size
    if(stage_num % 2 == 0)
    {
        out_val = buffer;
        out_next = out;
...
static void _vec_split_c2r_1d_f16 (riscv_vec_cpx_f16_t * FUNC_RESTRICT dst,
                                   ...
                                   q31_t ncfft)
{
    ...
    q31_t size = ncfft;
    riscv_vec_cpx_f16_t tmp_cal_1, tmp_cal_2, tmp_cal_wp;
    float16_t *wp_tbl_rifft_r = NULL, *wp_tbl_rifft_i = NULL;
    q31_t tmp_q = 0;
    float16_t div2 = (float16_t) 0.5f;
 ...
 * rifft_f16
 */
void  riscv_vec_rifft_f16 (float16_t * FUNC_RESTRICT out,
                           riscv_vec_cpx_f16_t * FUNC_RESTRICT in,
                           riscv_vec_rfft_cfg_f16_t cfg)
{
    if( cfg != NULL)
    {
        riscv_vec_cpx_f16_t * tmpbuf1 = cfg->buffer;
        riscv_vec_cpx_f16_t * tmpbuf2 = cfg->buffer + cfg->ncfft;

        _vec_split_c2r_1d_f16 (tmpbuf1, in, cfg->super_twiddles, cfg->ncfft);
        riscv_vec_inverse_butterfly_f16 ( (riscv_vec_cpx_f16_t*) out, tmpbuf1, cfg->factors, cfg->twiddles, tmpbuf2);

    }
}


void riscv_vec_rifft2d_f16 (float16_t * FUNC_RESTRICT out,
                            riscv_vec_cpx_f16_t * FUNC_RESTRICT in,
                            riscv_vec_rfft2d_cfg_f16_t cfg)
{
    ...
    riscv_vec_cpx_f16_t *tmp_in = (riscv_vec_cpx_f16_t*)&cfg->tmp_1d_input[0];
    riscv_vec_cpx_f16_t *data_in = NULL;
    float16_t *data_out = NULL;
    riscv_vec_cpx_f16_t *cp_data_in = in;
    q31_t m = cfg->m;
    q31_t n = cfg->ncfft;
    if( cfg != NULL)
    {
        ...
        for(tmp_idx = 0; tmp_idx < n + 1 ; tmp_idx++)
        {
            ...
            riscv_vec_inverse_butterfly_f16 (tmp_out, data_in, cfg->factors_m, cfg->twiddles_m, tmp_1d);
        }
        ...
        for(tmp_idx = 0 ; tmp_idx < m ; tmp_idx++)
        {
            ...
            data_in = tmp_in;
            riscv_vec_cpx_f16_t * tmp_split = cfg-> tmp_split;
            ...
            _vec_split_c2r_1d_f16 (tmp_split, data_in, cfg->super_twiddles, cfg->ncfft);
            riscv_vec_inverse_butterfly_f16 ((riscv_vec_cpx_f16_t*) data_out, tmp_split, cfg->factors_n, cfg->twiddles_n, tmp_1d);
        }
