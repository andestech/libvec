...
void riscv_vec_lfir_q15(const riscv_vec_lfir_q15_t * FUNC_RESTRICT instance,
                        q15_t * FUNC_RESTRICT src, q15_t * FUNC_RESTRICT dst, uint32_t size)
{
    while (size != 0)
    {
        ...
        /* f1(n) = f0(n) + K1 * g0(n-1) */
        /* g1(n) = f0(n) * K1 + g0(n-1) */
        fnext = (fcurr) + (((*pk) * gcurr) >> 15u);  
        gnext = ((fcurr * (*pk++)) >> 15u) + (gcurr); 
        *px++ = NDS_ISA_SATS(fcurr, 16u);
        fcurr = NDS_ISA_SATS((fnext), 16u);
        stage = instance->stage - 1;
        /* stage loop */
        while (stage != 0)
        {
            gcurr = *px;
            *px++ = NDS_ISA_SATS((gnext), 16u);
            fnext = (fcurr) + (((*pk) * gcurr) >> 15u); 
            gnext = ((fcurr * (*pk++)) >> 15u) + (gcurr);
            fcurr = NDS_ISA_SATS((fnext), 16u);
