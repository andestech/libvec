...
void riscv_vec_spafir_q15(riscv_vec_spafir_q15_t * FUNC_RESTRICT instance, q15_t * FUNC_RESTRICT src,
                          q15_t * FUNC_RESTRICT dst, q15_t * FUNC_RESTRICT buf, q31_t * FUNC_RESTRICT buf2, uint32_t size)
{
    q15_t *state = instance->state;
    q15_t *coeff = instance->coeff;
    q15_t *px, *py = state, *pb = buf;
    q15_t *out = dst;
    ...
    nds_cirwrite_q15(py, delsize, &instance->index, 1,  src, 1, size);
    ...
    py = state;
    nds_cirread_q15(py, delsize, &rinx, 1, pb, pb, size, 1, size);
    px = pb;
    buf2 = pScr2;
    blkcnt = size;
    while (blkcnt != 0u)
    ...
    while (blkcnt != 0u)
    {
        *out++ = (q15_t) NDS_ISA_SATS(*pScr2++ >> 15, 16u);
