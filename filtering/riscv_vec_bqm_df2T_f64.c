...
void riscv_vec_bqm_df2T_f64(const riscv_vec_bqm_df2T_f64_t * FUNC_RESTRICT instance,
                            float64_t * FUNC_RESTRICT src, float64_t * FUNC_RESTRICT dst, uint32_t size)
{
    ...
    {
        float64_t *in_t = in + t;
        float64_t *out_t = out + t;

        float64_t *coeffTmp = coeff + t;
        float64_t *stateTmp = state + t;
...
void riscv_vec_bqm_df2T_init_f64(riscv_vec_bqm_df2T_f64_t *instance, uint32_t nstage, float64_t *state, float64_t *coeff, uint32_t nch)
{
    ...
    instance->state = state; //(float64_t *) NDSV_MALLOC (state_memneeded * sizeof(float64_t));
    memset(instance->state, 0, state_memneeded * sizeof(float64_t));

    instance->coeff = (float64_t *) NDSV_MALLOC (coeff_memneeded * sizeof(float64_t));
    ...
    float64_t *coeff_tmp = instance->coeff;
