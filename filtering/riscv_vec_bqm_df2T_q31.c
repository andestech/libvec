...
void riscv_vec_bqm_df2T_q31(const riscv_vec_bqm_df2T_q31_t * FUNC_RESTRICT instance,
                            q31_t * FUNC_RESTRICT src, q31_t * FUNC_RESTRICT dst, uint32_t size)
{
    ...
    for(int t = 0 ; t < ch ; t++)
    {
        ...
        for(int s = 0 ; s < stage ; s++)
        {
            ...
            q63_t tmpd0 = (q63_t)d0 << lShift;
            q63_t tmpd1 = (q63_t)d1 << lShift;

            sample = size;
            while (sample > 0U)
            {
                xn = *in_t;
                acc1 = (q63_t)b0 * xn + tmpd0;
                acc1 = acc1 >> lShift;
                q31_t tmpacc = (q31_t)acc1;
                tmpd0 = (q63_t)b1 * xn + tmpd1;
                tmpd0 += (q63_t)a1 * tmpacc;
                tmpd1 = (q63_t)b2 * xn;
                tmpd1 += (q63_t)a2 * tmpacc;
                *out_t = tmpacc;
            ...
            *stateTmp = tmpd0 >> lShift; stateTmp+=ch;
            *stateTmp = tmpd1 >> lShift; stateTmp+=ch;
