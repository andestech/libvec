...
int riscv_vec_rmcmat_gemm_q15_q63(const MM_TYPE * FUNC_RESTRICT src1, const MM_TYPE * FUNC_RESTRICT src2, MM_TYPE_VQ * FUNC_RESTRICT dst,
                                const MM_TYPE alpha, const MM_TYPE beta,
                                uint32_t row, uint32_t col, uint32_t col2, int32_t shift)
{
    /**
     * complex_gemm_q15_q63
     ...
     * The 34.30 accumulated result is multiplied by a 1.15 (alpha), and the final output is saturated into 34.30 format.
     */
    int shift1 = (sizeof(MM_TYPE) << 3) - 1;
    vec_cgemm_purec_original_vq(src1, src2, dst, alpha, beta, row, col, col2, shift1);// (q30 * q15 + q15 * q30) >> shift1
    return NDSV_OK;
}
