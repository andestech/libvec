...
int riscv_vec_rmcmat_gemm_q7(const q7_t * FUNC_RESTRICT src1, const q7_t * FUNC_RESTRICT src2, q7_t * FUNC_RESTRICT dst,
                             ...
                             uint32_t row, uint32_t col, uint32_t col2, int32_t shift)
{
    ...
    do
    {
        ...
        do
        {
            ...
            q63_t temp = (q63_t)((((q63_t)*dst * beta) << 7) + (q63_t)alpha * sum_re);// (q14 << 7) + q21
            *dst = NDS_ISA_SATS((temp >> shift1), 8u);
            dst++;
            temp = (q63_t)((((q63_t)*dst * beta) << 7) + (q63_t)alpha * sum_im);// (q14 << 7) + q21
            *dst = NDS_ISA_SATS((temp >> shift1), 8u);
            dst++;
