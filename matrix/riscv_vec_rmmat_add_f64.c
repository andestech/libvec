...
int riscv_vec_rmmat_add_f64(const float64_t * FUNC_RESTRICT src1, const float64_t * FUNC_RESTRICT src2,
                            float64_t * FUNC_RESTRICT dst, uint32_t row, uint32_t col)
{
    uint32_t size = row * col;
    riscv_vec_add_f64(dst, (float64_t *)src1, (float64_t *)src2, size);
    return NDSV_OK;
}
