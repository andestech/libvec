...
int riscv_vec_rmmat_add_f16(const float16_t * FUNC_RESTRICT src1, const float16_t * FUNC_RESTRICT src2,
                            float16_t * FUNC_RESTRICT dst, uint32_t row, uint32_t col)
{
    uint32_t size = row * col;
    riscv_vec_add_f16(dst, (float16_t *)src1, (float16_t *)src2, size);
    return NDSV_OK;
}
