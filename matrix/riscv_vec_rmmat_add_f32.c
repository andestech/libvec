...
int riscv_vec_rmmat_add_f32(const float32_t * FUNC_RESTRICT src1, const float32_t * FUNC_RESTRICT src2,
                            float32_t * FUNC_RESTRICT dst, uint32_t row, uint32_t col)
{
    uint32_t size = row * col;
    riscv_vec_add_f32(dst, (float32_t *)src1, (float32_t *)src2, size);
    return NDSV_OK;
}
