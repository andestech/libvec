...
int riscv_vec_rmcmat_mul_q15(const q15_t * FUNC_RESTRICT src1, const q15_t * FUNC_RESTRICT src2, q15_t * FUNC_RESTRICT dst, uint32_t row, uint32_t col, uint32_t col2)
{
    /**
     * cmat_mul_q15
     ...
     * The final output is saturated into 1.15 format.
     */
    uint32_t j;
    ...
    do
    {
        ...
        do
        {
            ...
            *dst++ = NDS_ISA_SATS((sum_re >> 15), 16u);
            *dst++ = NDS_ISA_SATS((sum_im >> 15), 16u);
