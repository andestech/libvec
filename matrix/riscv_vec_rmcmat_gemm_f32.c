...
typedef float32_t MM_TYPE;//define internal_vec_mat_mul_tiling.h's data type
#include "internal_vec_mat_mul_tiling.h"

extern int riscv_vec_mulc_f32(float32_t * FUNC_RESTRICT dst, float32_t * FUNC_RESTRICT src, const float32_t cst, uint32_t count);

/* function description */
int riscv_vec_rmcmat_gemm_f32(const MM_TYPE * FUNC_RESTRICT src1, const MM_TYPE * FUNC_RESTRICT src2, MM_TYPE * FUNC_RESTRICT dst,
...
#endif
    const uint32_t count = (row * col2) << 1;
    float32_t *dst2 = dst;
    riscv_vec_mulc_f32(dst2, dst, beta, count);
