...
int riscv_vec_maddc_f32x4(riscv_vec_f32x4_t * FUNC_RESTRICT dst, riscv_vec_f32x4_t * FUNC_RESTRICT src, const riscv_vec_f32x4_t * FUNC_RESTRICT cst, uint32_t count)
{
    ...
    for ( unsigned int itr = 0; itr < count; itr++ )
    {
        ...
        dst[ itr ].w = dst[ itr ].w + (src[ itr ].w * cst->w);
    }
    return NDSV_OK;
}
