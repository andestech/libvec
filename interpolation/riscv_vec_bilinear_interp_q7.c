...
void riscv_vec_bilinear_interp_q7(riscv_vec_bilinear_interp_q7_t * FUNC_RESTRICT instance,
                                  q31_t * FUNC_RESTRICT src_x, q31_t * FUNC_RESTRICT src_y, q7_t * FUNC_RESTRICT dst, uint32_t size)
{
    ...
    for(uint32_t cnt = 0; cnt < size; cnt++)
    {
        ...
        else
        {
            ...
            out = ((x1 * (COE_LOWER_Q20 - xfract)));
            acc = (((q63_t) out * (COE_LOWER_Q20 - yfract)));

            /* x2 * (xfract) * (1-yfract)  in 2.22 and adding to acc */
            out = ((x2 * (COE_LOWER_Q20 - yfract)));
            acc += (((q63_t) out * (xfract)));

            /* y1 * (1 - xfract) * (yfract)  in 2.22 and adding to acc */
            out = ((y1 * (COE_LOWER_Q20 - xfract)));
