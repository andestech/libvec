...
void riscv_vec_spline_interp_init_f32(riscv_vec_spline_interp_f32_t * FUNC_RESTRICT instance,
                                      riscv_vec_spline_bound_cond cond,
                                      const float32_t * FUNC_RESTRICT srcx,
                                      const float32_t * FUNC_RESTRICT srcy,
                                      uint32_t nsize,
                                      float32_t * FUNC_RESTRICT coeffs,
                                      float32_t * FUNC_RESTRICT tmpbuf)
{
    ...
    if(cond == RISCV_VEC_SPLINE_NATURAL)
        u[0] = 0;  /* a(1,2) = 0 */
    else if(cond == RISCV_VEC_SPLINE_PARABOLIC_RUNOUT)
        u[0] = -1; /* a(1,2) = -1 */
...
#else
    nds_empty_func7(instance, cond, srcx, srcy, nsize, coeffs, tmpbuf);
#endif
}
