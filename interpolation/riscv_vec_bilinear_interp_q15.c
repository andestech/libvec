...
void riscv_vec_bilinear_interp_q15(riscv_vec_bilinear_interp_q15_t * FUNC_RESTRICT instance,
                                   q31_t * FUNC_RESTRICT src_x, q31_t * FUNC_RESTRICT src_y, q15_t * FUNC_RESTRICT dst, uint32_t size)
{
    ...
    for(uint32_t cnt = 0; cnt < size; cnt++)
    {
        ...
        else
        {
            ...
            out = (q31_t) (((q63_t) x1 * (COE_LOWER_Q20 - xfract)) >> 4U);
            acc = ((q63_t) out * (COE_LOWER_Q20 - yfract));

            /* x2 * (xfract) * (1-yfract)  in 1.51 and adding to acc */
            out = (q31_t) (((q63_t) x2 * (COE_LOWER_Q20 - yfract)) >> 4U);
            acc += ((q63_t) out * (xfract));

            /* y1 * (1 - xfract) * (yfract)  in 1.51 and adding to acc */
            out = (q31_t) (((q63_t) y1 * (COE_LOWER_Q20 - xfract)) >> 4U);
