...
static inline uintptr_t riscv_vec_align_size (uintptr_t sz, q31_t n)
{
    return (sz + n - 1) & -n;
}
        ...
        q31_t channels)
{
    ...
    int dirty = 1;
#endif
    q31_t bufstep = (q31_t) riscv_vec_align_size (dstw, 64); //align 64-byte for cache friendly
    ...
    for (k = 0; k < ksize; k++)
    {
        ...
        rows[k] = (q31_t*) (uintptr_t)riscv_vec_align_size ((uintptr_t)buf, 64) + bufstep * k;
    }
    ...
    for (dy = 0; dy < dsth; dy++, beta += ksize)
    {
        ...
        for (k = 0; k < ksize; k++)
        {
            q31_t sy = riscv_vec_clip (sy0 - ksize2 + 1 + k, 0, srch);//sy0=0~srch-1, k=0~1
...
void riscv_vec_img_resize_bilinear_rgba_u8(u8_t* FUNC_RESTRICT dst,
        u32_t dst_width,
        u32_t dst_height,
        u8_t* src,
        u32_t src_width,
