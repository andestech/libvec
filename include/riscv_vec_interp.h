...
#ifndef __RISCV_VEC_INTERP_H__
#define __RISCV_VEC_INTERP_H__
 ...
 *
 * Andes Vector library supports the linear interpolation functions for single- and half-precision floating-point, Q31, Q15 and Q7 data types. These functions are introduced in the subsections below.
 */
 ...
 * @return Always zero.
 */
void riscv_vec_linear_interp_f32(riscv_vec_linear_interp_f32_t *instance, float32_t *src, float32_t *dst, uint32_t size);
 ...
 * @return Always zero.
 */
void riscv_vec_linear_interp_f16(riscv_vec_linear_interp_f16_t *instance, float16_t *src, float16_t *dst, uint32_t size);
#endif
 ...
 * @return Always zero.
 */
void riscv_vec_linear_interp_q15(riscv_vec_linear_interp_q15_t * instance, q31_t * src, q15_t * dst, uint32_t size);
 ...
 * @return Always zero.
 */
void riscv_vec_linear_interp_q31(riscv_vec_linear_interp_q31_t * instance, q31_t * src, q31_t * dst, uint32_t size);
 ...
 * @return Always zero.
 */
void riscv_vec_linear_interp_q7(riscv_vec_linear_interp_q7_t * instance, q31_t * src, q7_t * dst, uint32_t size);
/** @} linear_interp */
 ...
 *
 * Andes Vector library supports the bilinear interpolation functions for single- and half-precision floating-point, Q31, Q15 and Q7 data types. These functions are introduced in the subsections below.
 */
...
}riscv_vec_bilinear_interp_f32_t;
 ...
 * @return Always zero.
 */
void riscv_vec_bilinear_interp_f32(riscv_vec_bilinear_interp_f32_t * instance, float32_t * src_x, float32_t * src_y, float32_t * dst, uint32_t size);
...
}riscv_vec_bilinear_interp_q31_t;
 ...
 * @return Always zero.
 */
void riscv_vec_bilinear_interp_q31(riscv_vec_bilinear_interp_q31_t * instance, q31_t * src_x, q31_t * src_y, q31_t * dst, uint32_t size);
...
}riscv_vec_bilinear_interp_q15_t;
 ...
 * @return Always zero.
 */
void riscv_vec_bilinear_interp_q15(riscv_vec_bilinear_interp_q15_t * instance, q31_t * src_x, q31_t * src_y, q15_t * dst, uint32_t size);
...
}riscv_vec_bilinear_interp_q7_t;
 ...
 * @return Always zero.
 */
void riscv_vec_bilinear_interp_q7(riscv_vec_bilinear_interp_q7_t * instance, q31_t * src_x, q31_t * src_y, q7_t * dst, uint32_t size);
#if defined (__riscv_zfh)
/**
...
}riscv_vec_bilinear_interp_f16_t;
 ...
 * @return Always zero.
 */
void riscv_vec_bilinear_interp_f16(riscv_vec_bilinear_interp_f16_t * instance, float16_t * src_x, float16_t * src_y, float16_t * dst, uint32_t size);
#endif
/** @} bilinear_interp */
 ...
 *
 * Spline interpolation is a method of interpolation where the interpolant is a piecewise-defined polynomial called "spline". You can use the function riscv_vec_spline_interp_init_f32 to calculate the coefficient of spline and call the function riscv_vec_spline_interp_f32 to get the interpolation result of the input vector.
 *
 * Andes Vector library only supports the spline interpolation functions for the F32 data type. The function is introduced in the subsections below.
 */
...
typedef enum
{
  RISCV_VEC_SPLINE_NATURAL = 0,           /**< Natural spline */
  RISCV_VEC_SPLINE_PARABOLIC_RUNOUT = 1   /**< Parabolic runout spline */
} riscv_vec_spline_bound_cond;
...
typedef struct
{
  riscv_vec_spline_bound_cond cond;   /**< Type (boundary conditions) */
 ...
 * @return none.
 */
void riscv_vec_spline_interp_init_f32(riscv_vec_spline_interp_f32_t * instance,
                                      riscv_vec_spline_bound_cond cond,
 ...
 * @return none.
 */
void riscv_vec_spline_interp_f32(riscv_vec_spline_interp_f32_t * instance, const float32_t * src, float32_t * dst, uint32_t size);
