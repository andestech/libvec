...
static inline void vec_cirwrite_f32(int32_t *buf, int32_t len,
...
static inline void vec_cirread_f32(int32_t *buf, int32_t len,
...
static inline void vec_cirwrite_q7(q7_t *buf, int32_t len,
...
// Floating-point Circular write function
static inline void vec_cirwrite_q15(q15_t *buf, int32_t len,
...
static inline void vec_cirread_q7(q7_t *buf, int32_t len,
...
static inline void vec_cirread_q15(q15_t *buf, int32_t len,
...
static inline q31_t vec_truncate_q63_to_q31(q63_t src)
{
#ifdef NDS_CPU_64
...
static inline int32_t vec_log2_32b(uint32_t v)
{
    uint32_t r, shift;
...
static inline uint32_t vec_ghb_32b(q31_t data)
{
    uint32_t count;
    ...
    if (data != 0)
    {
        count = vec_log2_32b(data) + 1;
    }
#endif
    return count;
}

static inline uint32_t vec_clz(q31_t data)
{
    uint32_t count;
#if defined(__NDS32_EXT_PERF__)
    count = __nds32__clz(data);
#else
    count = 32 - vec_ghb_32b(data);
#endif
    return count;
}

static inline uint32_t vec_recip_q15(q15_t src, q15_t *dst)
{
    q15_t out = 0;
    uint32_t val = 0, sign = 0;

    sign = (src > 0) ? (vec_clz(src) - 17) : (vec_clz(-src) - 17);
    src = src << sign;
...
static inline uint32_t vec_recip_q31(q31_t src, q31_t *dst)
{
    q31_t out = 0;
    uint32_t val = 0, sign = 0;

    sign = (src > 0) ? (vec_clz(src) - 1) : (vec_clz(-src) - 1);
    src = src << sign;
    ...
    val = 0x7FFFFFFF - val;
    out = (q31_t) vec_truncate_q63_to_q31(((q63_t) out * val) >> 30u);
