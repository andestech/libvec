...
static inline void vec_estperf(uint64_t tmpinst, uint64_t tmpcycle,
...
static inline int vec_indpwr2(const int target)
{
    int temp = target - 1;
    temp |= temp >> 1;
    temp |= temp >> 2;
...
static inline void vec_et_keepB_2x2x64_slice(uint32_t row, uint32_t *row_slice, uint32_t *col2_slice)
{
    ...
    else
    {
        ...
        else
        {
            *col2_slice = 64;
            ...
            a_slice = vec_indpwr2(a_slice);
            *row_slice = a_slice;
        }
    }
}

static inline void vec_et_keepB_8x4x16_slice(uint32_t row, uint32_t *row_slice, uint32_t *col2_slice)
{
    uint32_t dcache_byte = DCACHE_BYTE;
    uint32_t total_byte = 2 * row * row * sizeof(float64_t);
    uint32_t tmp;
...
static inline void vec_et_keepB_4x4x32_slice(uint32_t row, uint32_t *row_slice, uint32_t *col2_slice)
{
    uint32_t dcache_byte = DCACHE_BYTE;
    uint32_t total_byte = 2 * row * row * sizeof(float64_t);
    uint32_t tmp;
